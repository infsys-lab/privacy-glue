...
class Reading_Comprehension_Pipeline(Privacy_GLUE_Pipeline):
        ...
        if self.train_args.do_predict:
            if self.data_args.max_predict_samples is not None:
                ...
                self.raw_datasets["test"] = self.raw_datasets["test"].select(
                    range(self.data_args.max_predict_samples)
                )
    ...
    def _postprocess_qa_predictions(
        ...
        for example_index, example in enumerate(tqdm(examples)):
            ...
            for feature_index in feature_indices:
                ...
                # answers that do not have the maximum context
                ...
                for start_index in start_indexes:
                    for end_index in end_indexes:
                        ...
                        # available (if such information is
                        # provided).
                        if (
            ...
            if not version_2_with_negative:
                ...
                if score_diff > self.data_args.null_score_diff_threshold:
    ...
    def _run_train_loop(self) -> None:
        ...
        # Training
        if self.train_args.do_train:
            train_result = trainer.train(resume_from_checkpoint=self.last_checkpoint)
            trainer.save_model()
        ...
        if self.train_args.do_predict:
            ...
            results = trainer.predict(self.predict_dataset, self.raw_datasets["test"])
