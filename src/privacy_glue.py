...
def train(model_args: ModelArguments, data_args: DataArguments,
    ...
    add_file_handler(LOGGER, training_args.get_process_log_level(),
    ...
    add_file_handler(datasets.utils.logging.get_logger(),
    ...
    add_file_handler(transformers.utils.logging.get_logger(),
                     training_args.get_process_log_level(),
                     os.path.join(training_args.output_dir, "session.log"))

    # check for existing exit code and decide action
    if os.path.exists(
            os.path.join(training_args.output_dir, EXIT_CODE_FILE)
    ) and not training_args.overwrite_output_dir and training_args.do_train:
    ...
    if training_args.do_train and not training_args.overwrite_output_dir:
        ...
        if checkpoint is not None:
            LOGGER.warning("Checkpoint detected, resuming training from %s. "
...
def main(model_args: ModelArguments, data_args: DataArguments,
         training_args: TrainingArguments) -> None:
    ...
    for task in tasks:
        for seed in range(model_args.random_seed_iterations):
            ...
            training_args.output_dir = os.path.join(
                model_dir, re.sub(r"[/-]", "_", data_args.task),
                "seed_%s" % seed)
